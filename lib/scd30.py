...
class SCD30:
    ...
    def measure_attempt(self):
        bcmd = struct.pack('>H', self.READ_MEASUREMENT)
        self.i2c.writeto(self.addr, bcmd)
        time.sleep_ms(20)
        measurement = self.i2c.readfrom(self.addr, 18)
        self.measurement = measurement
        value = measurement[0:]
        return struct.unpack('>f', value[0:2] + value[3:5])[0]

    def read_measurement(self):
              
        attempts = 40
        i = 0
        while (math.isnan(self.measure_attempt())) and (i<attempts):
            time.sleep_ms(100)
            i += 1
